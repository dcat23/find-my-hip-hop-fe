...
import { deleteTeam } from "@dcat23/features/team";
import va from "@vercel/analytics";

export function DeleteTeamForm({ teamName }: { teamName: string }) {
  const { id } = useParams() as { id: string };
  const router = useRouter();
  return (
    <form
      action={async (data: FormData) =>
        window.confirm("Are you sure you want to delete your team?") &&
        deleteTeam(data, id, "delete")
          .then(async (res) => {
            if (res.error) {
              toast.error(res.error);
            } else {
              va.track("Deleted Team");
              router.refresh();
              router.push("/teams");
    ...
    >
      <div className="relative flex flex-col space-y-4 p-5 sm:p-10">
        <h2 className="font-cal text-xl dark:text-white">Delete Team</h2>
        <p className="text-sm text-stone-500 dark:text-stone-400">
          Deletes your team and all notes associated with it. Type in the name
          of your team <b>{teamName}</b> to confirm.
        </p>
