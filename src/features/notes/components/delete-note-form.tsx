...
import { deleteNote } from "@dcat23/features/notes";
import va from "@vercel/analytics";

export function DeleteNoteForm({ noteName }: { noteName: string }) {
  ...
  return (
    <form
      action={async (data: FormData) =>
        ...
        deleteNote(data, id, "delete").then((res) => {
          if (res.error) {
            toast.error(res.error);
          } else {
            va.track("Deleted Note");
            router.refresh();
    ...
    >
      <div className="relative flex flex-col space-y-4 p-5 sm:p-10">
        <h2 className="font-cal text-xl dark:text-white">Delete Note</h2>
        <p className="text-sm text-stone-500 dark:text-stone-400">
          Deletes your note permanently. Type in the name of your note{" "}
          <b>{noteName}</b> to confirm.
        </p>
